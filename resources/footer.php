<head>
    <link rel="stylesheet" href="/RompeMalla/css/footer.css">
    <link href="https://fonts.googleapis.com/css2?family=PT+Serif&display=swap" rel="stylesheet">
</head>

<footer>
    <div class="fcontainer">
  
        <div class="brand">
            <img src="/RompeMalla/img/LOGO PAWTS.png" alt="Logo Paws">
        </div>

  
        <div class="links">
            <a href="/RompeMalla/pages/privacidad.php">Política de Privacidad</a>
            <a href="/RompeMalla/pages/historia.php">Sobre Nosotros</a>
            <a href="/RompeMalla/pages/contacto.php">Contáctanos</a>
            <a href="/RompeMalla/pages/FAQ.php">Centro de Ayuda</a>
        </div>

  
        <div class="socials">
            <h3>!Siguenos!</h3>
            <a href="https://www.tiktok.com">
                <img src="/RompeMalla/img/Social Icons tiktok.png" alt="TikTok">
            </a>
            <a href="https://www.instagram.com/pawtsmascotas/">
                <img src="/RompeMalla/img/Social Icons instagram.png" alt="Instagram">
            </a>
        </div>
    </div>

    <div class="copyright">
        <p>Copyright PAWTS. © Todos los derechos reservados.</p>
    </div>
</footer>
